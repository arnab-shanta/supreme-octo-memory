...
class DatabaseHandler(context: Context) : SQLiteOpenHelper(context, DatabaseHandler.DB_NAME, null, DatabaseHandler.DB_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        val CREATE_TABLE = "CREATE TABLE $TABLE_NAME ($ID INTEGER PRIMARY KEY, $NAME TEXT, $DESC TEXT, $PRIORITY TEXT, $COMPLETED TEXT);"
    ...
    fun getAllTasks(): List<Tasks> {
        ...
        val db = writableDatabase
        val selectQuery = "SELECT  * FROM $TABLE_NAME ORDER BY $COMPLETED"
        val cursor = db.rawQuery(selectQuery, null)
        if (cursor != null) {
